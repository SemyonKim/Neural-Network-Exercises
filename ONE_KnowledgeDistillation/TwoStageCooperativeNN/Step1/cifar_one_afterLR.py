...
from utils import Bar, Logger, AverageMeter, accuracy, mkdir_p, savefig, ramps, GradientRatioScheduler, print_model
import pdb
...
def main():
    ...
    if args.resume or args.evaluate:
        ...
        logger.set_names(['Epoch', 'TAcc_1', 'VAcc_1','TAcc_2','VAcc_2','TAcc_3','VAcc_3','TAcc_4','VAcc_4','TAcc_5','VAcc_5','TAcc_e','VAcc_e', 'Time', 'Learning Rate'])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        #train_loss, train_acc_1,train_acc_2,train_acc_3,train_acc_4,train_acc_5,train_acc_en = train(trainloader, model, criterion, criterion_kl, optimizer, epoch,use_cuda)
        #test_loss, test_acc_1,test_acc_2,test_acc_3,test_acc_4,test_acc_5,test_acc_en = test(testloader, model, use_cuda)
        st = time.time()
        train_loss, train_acc_1,train_acc_2,train_acc_3,train_acc_4,train_acc_5,train_acc_en = train(trainloader, model, criterion, criterion_kl, optimizer, scheduler, epoch, use_cuda)
        test_loss, test_acc_1,test_acc_2,test_acc_3,test_acc_4,test_acc_5,test_acc_en = test(testloader, model, use_cuda)

        # append logger file
        logger.append([epoch, train_acc_1, test_acc_1,train_acc_2,test_acc_2,train_acc_3,test_acc_3,train_acc_4,test_acc_4,train_acc_5,test_acc_5,train_acc_en,test_acc_en, time.time()-st, scheduler.get_lr()])
...
def train(trainloader, model, criterion, criterion_kl, optimizer, scheduler, epoch, use_cuda):
    ...
    top1_c4 = AverageMeter()
    top5_c4 = AverageMeter()
    top1_c5 = AverageMeter()
    top5_c5 = AverageMeter()
    top1_t = AverageMeter()
    top5_t = AverageMeter()
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        loss_cross = criterion(outputs1, targets) + criterion(outputs2, targets) + criterion(outputs3,targets) + criterion(
                 outputs4, targets)+ criterion(outputs5,targets) + criterion(outputs6, targets)

        loss_kl = consistency_weight*(criterion_kl(outputs1, outputs6) +criterion_kl(outputs2,outputs6)+criterion_kl(outputs3,outputs6)+criterion_kl(outputs4,outputs6)+criterion_kl(outputs5,outputs6))
        prec1_t, prec5_t = accuracy(outputs6.data, targets.data, topk=(1, 5))
        ...
        prec1_c4, prec5_c4 = accuracy(outputs4.data, targets.data, topk=(1, 5))
        prec1_c5, prec5_c5 = accuracy(outputs5.data, targets.data, topk=(1, 5))
        ...
        top1_c4.update(prec1_c4.item(), inputs.size(0))
        top5_c4.update(prec5_c4.item(), inputs.size(0))
        top1_c5.update(prec1_c5.item(), inputs.size(0))
        top5_c5.update(prec5_c5.item(), inputs.size(0))
        ...
        bar.suffix = '({batch}/{size})   || Loss: {loss:.4f} |LossKL: {losses_kl:.4f} | top1_C1: {top1_C1: .4f} | top1_C2: {top1_C2: .4f}|top1_C3: {top1_C3: .4f}| top1_C4: {top1_C4: .4f}|top1_C5: {top1_C5: .4f}| top1_t:{top1_t: .4f} | top5_C1: {top5_C1: .4f} | top5_C2: {top5_C2: .4f}|top5_C3: {top5_C3: .4f}| top5_C4: {top5_C4: .4f}|top5_C5: {top5_C5: .4f}| top5_t: {top5_t: .4f} '.format(
            batch=batch_idx + 1,
        ...
        bar.next()
    bar.finish()
    return (losses.avg,top1_c1.avg,top1_c2.avg,top1_c3.avg,top1_c4.avg,top1_c5.avg,top1_t.avg)
...
def test(testloader, model, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        # measure accuracy and record loss
            ...
            prec1_c4, _ = accuracy(outputs4.data, targets.data, topk=(1, 5))
            prec1_c5, _ = accuracy(outputs5.data, targets.data, topk=(1, 5))
            prec1_en, _ = accuracy(outputs6.data, targets.data, topk=(1, 5))
            top1_c1.update(prec1_c1.item(), inputs.size(0))
            top1_c2.update(prec1_c2.item(), inputs.size(0))
            top1_c3.update(prec1_c3.item(), inputs.size(0))
            top1_c4.update(prec1_c4.item(), inputs.size(0))
            top1_c5.update(prec1_c5.item(), inputs.size(0))
            top1_avg.update((prec1_c1.item()+prec1_c2.item()+prec1_c3.item()+prec1_c4.item()+prec1_c5.item())/5, inputs.size(0))
        ...
        # plot progress
        bar.suffix = '({batch}/{size})| Loss: {loss: .4f} | top1_C1: {top1_C1: .4f} |top1_C2: {top1_C2: .4f}|top1_C3: {top1_C3:.4f} |top1_C4: {top1_C4: .4f}|top1_C5: {top1_C5:.4f} |top1_t: {top1_t: .4f}'.format(
            batch=batch_idx + 1,
            size=len(testloader),
            loss=losses.avg,
            top1_C1=top1_c1.avg,
