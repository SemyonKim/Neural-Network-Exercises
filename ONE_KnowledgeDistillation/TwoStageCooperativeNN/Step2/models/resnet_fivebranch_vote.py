...
class ONE_ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Voting combinations
        combos = []
        # Example: 4-branch combos
        combos.append(weights[0]*branches[0] + weights[1]*branches[1] + weights[2]*branches[2] + weights[3]*branches[3])
        combos.append(weights[0]*branches[0] + weights[1]*branches[1] + weights[2]*branches[2] + weights[4]*branches[4])
        # ... (continue for all combinations as in your original loose code)

        # Single branch weighted outputs
        combos.extend([weights[i]*branches[i] for i in range(5)])

        return branches[0], branches[1], branches[2], branches[3], branches[4], x_m, combos
