"""
train_fivebranch_vote.py

Training script for CIFAR-10/100 using ONE_ResNet with 5 branches + voting ensembles.
"""
...
parser = argparse.ArgumentParser(description="PyTorch CIFAR10/100 Training (5 branches + voting)")
...
parser.add_argument("--resume", default="", type=str, help="path to latest 5-branch voting checkpoint")
...
def main():
    ...
    # Model
    print(f"==> creating model '{args.arch}' with 5 branches + voting")
    ...
    logger = Logger(os.path.join(args.checkpoint, "log.txt"), title="cifar_fivebranch_vote")
    logger.set_names([
    ...
    if args.evaluate:
        print("\nEvaluation only")
        _, acc1, acc2, acc3, acc4, acc5, acc_en, acc_votes = test(testloader, model)
        print(f"Branches: {acc1:.2f}, {acc2:.2f}, {acc3:.2f}, {acc4:.2f}, {acc5:.2f}")
        print(f"Ensemble: {acc_en:.2f}")
        print("Voting combos:", acc_votes)
        return
    ...
    for epoch in range(args.epochs):
        ...
        _, acc1, acc2, acc3, acc4, acc5, acc_en, _ = train(trainloader, model, criterion, criterion_kl, optimizer, epoch)
        _, tacc1, tacc2, tacc3, tacc4, tacc5, tacc_en, _ = test(testloader, model)
...
def train(trainloader, model, criterion, criterion_kl, optimizer, epoch):
    model.train()
    losses = AverageMeter()
    top1_c1, top1_c2, top1_c3, top1_c4, top1_c5, top1_en = [AverageMeter() for _ in range(6)]
    ...
    for inputs, targets in trainloader:
        ...
        outputs1, outputs2, outputs3, outputs4, outputs5, outputs6, _ = model(inputs)
    ...
    return losses.avg, top1_c1.avg, top1_c2.avg, top1_c3.avg, top1_c4.avg, top1_c5.avg, top1_en.avg, None

def test(testloader, model):
    model.eval()
    losses = AverageMeter()
    top1_c1, top1_c2, top1_c3, top1_c4, top1_c5, top1_en = [AverageMeter() for _ in range(6)]
    top1_votes = []  # list of AverageMeter objects for voting combos
    ...
    with torch.no_grad():
        for inputs, targets in testloader:
            ...
            # Forward pass: model returns 5 branches, ensemble, and voting combos
            outputs1, outputs2, outputs3, outputs4, outputs5, outputs6, outputs6m = model(inputs)

            # Branch + ensemble accuracies
            ...
            # Voting combos accuracies
            if not top1_votes:  # initialize meters on first batch
                top1_votes = [AverageMeter() for _ in range(len(outputs6m))]

            for i, out in enumerate(outputs6m):
                prec_vote, _ = accuracy(out.data, targets.data, topk=(1, 5))
                top1_votes[i].update(prec_vote.item(), inputs.size(0))

    # Collect averages for voting combos
    acc_votes = [meter.avg for meter in top1_votes]

    return losses.avg, top1_c1.avg, top1_c2.avg, top1_c3.avg, top1_c4.avg, top1_c5.avg, top1_en.avg, acc_votes
